...
function loadFromStorage() {
    try {
        ...
        if (savedOperai) {
            operai = JSON.parse(savedOperai);
            console.log('📋 Operai caricati da localStorage');
        }
        if (savedCantieri) {
            cantieri = JSON.parse(savedCantieri);
            console.log('🏗️ Cantieri caricati da localStorage');
        }
        if (savedEmailConfig) {
            emailConfig = {...emailConfig, ...JSON.parse(savedEmailConfig)};
            console.log('📧 Configurazione email caricata da localStorage');
        }
        if (savedGeneralConfig) {
            generalConfig = {...generalConfig, ...JSON.parse(savedGeneralConfig)};
            console.log('⚙️ Configurazione generale caricata da localStorage');
        }
    } catch (error) {
...
// ===== MIGLIORATE FUNZIONI MODAL =====
function openModal(modalId) {
    console.log(`🔓 Apertura modal: ${modalId}`);
    const modal = document.getElementById(modalId);
    if (modal) {
        modal.classList.remove('hidden');
        // Aggiunge event listener per chiusura con ESC
...
function setupEventListeners() {
    ...
    if (searchOperai) {
        searchOperai.addEventListener('input', function(e) {
    ...
    if (searchCantieri) {
        searchCantieri.addEventListener('input', function(e) {
            const searchTerm = e.target.value.toLowerCase();
            filterCantieri(searchTerm);
        });
    }

    // Auto-save su modifiche
    document.addEventListener('change', function(e) {
        if (e.target.form && (e.target.form.id === 'form-operaio' || e.target.form.id === 'form-cantiere')) {
            setTimeout(saveToStorage, 100); // Salva dopo 100ms
        }
    });

    // Gestione click outside modal per chiudere
...
function filterCantieri(searchTerm) {
    ...
    cantieriElements.forEach(cantiere => {
        const text = cantiere.textContent.toLowerCase();
        if (text.includes(searchTerm)) {
...
// ===== MIGLIORAMENTO DRAG & DROP =====
function setupDragAndDrop() {
    console.log('🔄 Setup drag and drop migliorato');

    // Rende draggable tutti gli operai
    document.addEventListener('click', function() {
        ...
        operaiCards.forEach(card => {
            ...
            card.addEventListener('dragstart', function(e) {
                draggedOperaio = parseInt(e.target.dataset.operaioId);
                e.target.style.opacity = '0.5';
                console.log(`🤏 Drag iniziato per operaio ID: ${draggedOperaio}`);
            });
    ...
    // Setup drop zones sui cantieri
    document.addEventListener('click', function() {
        const cantieri = document.querySelectorAll('.cantiere');
        cantieri.forEach(cantiere => {
            cantiere.addEventListener('dragover', function(e) {
                e.preventDefault();
                e.target.style.backgroundColor = 'rgba(33, 128, 141, 0.2)';
            });

            cantiere.addEventListener('dragleave', function(e) {
                e.target.style.backgroundColor = '';
            });

            cantiere.addEventListener('drop', function(e) {
                e.preventDefault();
                e.target.style.backgroundColor = '';

                const cantiereId = parseInt(e.target.dataset.cantiereId);
                if (draggedOperaio && cantiereId) {
                    assignOperaioToCantiere(draggedOperaio, cantiereId);
                    draggedOperaio = null;
                }
            });
        });
